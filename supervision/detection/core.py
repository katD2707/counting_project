...
class Detections:
    ...
    def from_yolox(cls, yolox_results) -> Detections:
        """
        Creates a Detections instance from a
        [YOLOx] inference result.

        Args:
            yolox_results (yolov5.models.common.Detections):
                The output Detections instance from YOLOx
        ...
        Example:
            ...
            >>> image = cv2.imread(SOURCE_IMAGE_PATH)
            >>> model = torch.hub.load('ultralytics/yolox', 'yolox')
            >>> result = model(image)
            >>> detections = sv.Detections.from_yolov5(result)
            ```
        """
